...
test("Create Storage Zone", async ({ page }) => {
  ...
  await page
    ...
    .fill("This is a shelf created for testing purposes");
...
test("test", async ({ page }) => {
  await page.getByRole("cell", { name: "ShelfTest" }).click();
  await page
    .getByRole("row", { name: "ShelfTest This is a shelf" })
    .locator("svg")
    .click();
  ...
  await page
    .getByPlaceholder("Description")
    .fill("This is a testing shelf located on SHR");
  await page.getByRole("button", { name: "Save & Exit" }).click();
  await page.getByRole("cell", { name: "ShelfTest" }).click();
});
...
test("CompleteStorageZone", async ({ page }) => {
  ...
  await page
    .getByPlaceholder("Description")
    .fill("Storage Legel located on SH1 level 1 R");
  await page.getByRole("button", { name: "Save & Exit" }).click();
});
